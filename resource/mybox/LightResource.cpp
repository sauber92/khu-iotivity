...
const char* PublicKeyPath = "keys/public_key.pem";
const char* PrivateKeyPath = "keys/private_key.pem";
std::string MyboxPassword = "Iotivity";
std::string PBK_KEY = "PUBLIC_KEY";
std::string PASSWORD_KEY = "PASSWORD";
std::string OPEN_KEY = "IS_OPEN";
...
LightResource::LightResource() :
	...
	ifstream ifs;
	ifs.open(PublicKeyPath, ios_base::in);
	std::string strTmp;
	std::string strKey = "";
	while (!ifs.eof()) {
		getline(ifs, strTmp);
	...
	m_lightRep.setValue(PBK_KEY, strKey);
...
OCRepresentation LightResource::put(OCRepresentation& rep) {
	OCRepresentation repRtn;
	std::string pwd;
	std::string decPwd = "";
	try {
		//Todo decrypt
		rep.getValue(PASSWORD_KEY, pwd);
		char* tmp = new char[pwd.size()];
		memcpy(tmp, pwd.c_str(), pwd.size());

		//decrypt password..
		ERR_load_crypto_strings();
		char b64priv_key[1024];
		loadPemKeyFromFile(PrivateKeyPath, b64priv_key);
		RSA *privKey = loadPRIVATEKeyFromString(b64priv_key);

		// Now we got the data at the server.  Time to decrypt it.
		int rBinLen;
		unsigned char* rBin = rsaDecryptThisBase64(privKey, tmp, &rBinLen);

		cout << "get password :: " << pwd << endl;

		cout << "========DECRYPTED DATA==========" << endl;
		cout << "size : " << rBinLen << endl;
		...
		free(rBin);
		ERR_free_strings();
		//decrypt password end

		for(int i=0 ; i<rBinLen ; i++)
			decPwd += rBin[i];

		if (decPwd == MyboxPassword) {
			system("sudo sh ecrypt.sh 3fb41f01104f5ae92bf20962fbe117ac door_encrypted door_decrypted");
			repRtn.setValue(OPEN_KEY, "Y");
		} else {
			repRtn.setValue(OPEN_KEY, "N");
		}
